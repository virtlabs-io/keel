-- tpcc_like_reconnect.lua
-- TPC-C inspired mixed workload using the pgbench schema.
-- RECONNECT VERSION: opens a fresh connection for every transaction, then
-- disconnects. Simulates PHP / short-lived client behaviour.  A connection
-- pooler absorbs the reconnect overhead; direct-to-postgres will quickly
-- exhaust ephemeral ports at high concurrency.
--
-- For a persistent-connection version see tpcc_like.lua.
--
...
--
-- Usage:
--   sysbench /keel/bench/tpcc_like_reconnect.lua \
...
--
--   READ_TXN_MODE=readonly sysbench /keel/bench/tpcc_like_reconnect.lua ...
...
function thread_done()
    -- nothing to close — connections are per-transaction
end
